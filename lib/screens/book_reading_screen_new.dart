...
class _BookReadingScreenState extends State<BookReadingScreen>
  ...
  List<WordAssessment> _wordAssessments = [];
  ...
  void _loadBookContent() {
    ...
    try {
      // Load pages from book chapters
      if (widget.book.chapters.isNotEmpty) {
        _pages = widget.book.chapters;
        for (int i = 0; i < widget.book.chapters.length; i++) {
          final chapter = widget.book.chapters[i];
          // Split into sentences for detailed reading
          final sentences = chapter
              .split(RegExp(r'[.!?]+'))
              .map((s) => s.trim())
      ...
      } else if (widget.book.pages.isNotEmpty) {
      ...
      if (_pages.isEmpty) {
        ...
        _sentences = ['This book appears to be empty'];
      }
  ...
  void _bookmarkCurrentPage() async {
    try {
      final updatedBook = Book(
        ...
        readingProgress: (_currentPageIndex / _pages.length) * 100,
